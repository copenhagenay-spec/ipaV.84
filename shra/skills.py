...
_SHRA_KOKORO_VOICE_DEFAULT = "af_heart"
...
def _kokoro_tts_play(text: str) -> None:
    ...
    try:
        ...
        kokoro = _get_kokoro()
        voice = load_config().get("tts_voice", _SHRA_KOKORO_VOICE_DEFAULT)
...
def _tts_speak(text: str, bypass_mute: bool = False) -> bool:
    if _shra_muted["value"] and not bypass_mute:
        return False
...
_CONVERSATIONAL_PREFIX = re.compile(
    r"^(hey shira[,]?\s*|shira[,]?\s*|hey there[,]?\s*|"
...
_shra_muted: dict = {"value": False, "status_fn": None}  # mute state + optional UI callback
...
_CLOSE_OVERRIDES = {
    ...
    "opera": "opera.exe",
    "notepad": "notepad.exe",
    "shira": None,  # handled via PID file — see _close_app
}
...
def _close_app(app_name: str) -> bool:
    ...
    if normalized in _norm_overrides:
        ...
        if target is None:
            # PID-based kill (e.g. shira — kills only this instance)
            try:
                pid_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "shra.pid")
                ...
                _log_event(f"CLOSE_SHRA_PID: {pid} -> rc={result.returncode}")
                return result.returncode == 0
            except Exception as exc:
                _log_event(f"CLOSE_SHRA_PID_FAILED: {exc}")
                return False
...
# --- Restart SH|RA ---
@_intent(990, r"(\b(restart|reboot)\s+(shira|assistant|the assistant)\b|^(restart|reboot)$)")
def _ih_restart_shra(m, t, allow_prompt, confirm_fn, restart_fn):
...
def _ih_mute(m, t, allow_prompt, confirm_fn, restart_fn):
    ...
    if _set_volume(0):
        _shra_confirm("volume")
    else:
        _shra_failure("volume")
    return True
...
# --- Mute / Unmute ---
@_intent(950, r"\b(mute|be quiet|shut up shira|silence|go quiet|stop talking)\b")
def _ih_mute(m, t, allow_prompt, confirm_fn, restart_fn):
    _shra_muted["value"] = True
    fn = _shra_muted.get("status_fn")
    if fn:
        fn("Muted")
    _log_event("SHRA_MUTED")
    return True


@_intent(951, r"\b(unmute|you can talk|start talking|come back|wake up shira|i can hear you now|okay shira|ok shira|shira come back|shira unmute)\b")
def _ih_unmute(m, t, allow_prompt, confirm_fn, restart_fn):
    _shra_muted["value"] = False
    fn = _shra_muted.get("status_fn")
    if fn:
        fn(None)  # None tells assistant.py to restore the real status label
    _tts_speak("I'm back.", bypass_mute=True)
    _log_event("SHRA_UNMUTED")
    return True
