"""Main UI layout for SH|RA (PySide6)."""
...
def _make_listbox(height_rows=6) -> QListWidget:
    ...
    lb.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
    return lb
...
def build_ui(window, state: dict, callbacks: dict, constants: dict):
    ...
    sidebar_vl.setContentsMargins(0, 52, 0, 8)
    ...
    def _content_row_resize(event):
        sidebar.setGeometry(0, 0, 48, content_row.height())
    ...
    title_lbl = QLabel("S H | R A  +" if is_premium() else "S H | R A")
    ...
    # Community
    for w in _section_label("Community", "Join the SH|RA Discord server."):
        home_vl.addWidget(w)
    ...
    # Listening Mode
    for w in _section_label("Listening Mode", "Choose how SH|RA listens for your voice commands."):
    ...
    voice_hint = _hint_label("  Choose SH|RA's voice (takes effect immediately)")
    ...
    confirm_desc = _hint_label("SH|RA will ask you to confirm before opening apps or running commands.")
    ...
    # Personality
    for w in _section_label("Personality", "Choose how SH|RA speaks to you."):
        settings_vl.addWidget(w)
    ...
    chatter_desc = _hint_label("SH|RA will speak unprompted after periods of inactivity")
    chatter_row = QWidget()
    ...
    def _activate_key():
        ...
        if _vk(_k):
            ...
            _c["license_key"] = _k
            _save_config_ui(_c)
            _key_status.setText("  Valid — restart SH|RA to activate")
    ...
    # Birthday
    for w in _section_label("Birthday", "SH|RA will wish you happy birthday on startup."):
        settings_vl.addWidget(w)
    ...
    util_vl.addWidget(_hrow(
        ...
        _danger_btn("Delete Cache", _clear_pycache),
    ))
...
_danger_btn("Remove Selected", _remove_app),
    ...
    if is_premium():
        ...
        _cm_locked = _hint_label("⬡ Command Macros — SH|RA+ only. Enter a license key above to unlock.")
        _cm_locked.setWordWrap(True)
        integ_vl.addWidget(_cm_locked)
        macros_textbox = _make_listbox(0)
        macro_pending_textbox = _make_listbox(0)
    ...
    status_frame.setStyleSheet(
        ...
        f"stop:0 rgba(26,26,26,0), stop:1 rgba(43,43,43,240)); "
    ...
    status_kw.setStyleSheet("font-weight: bold; font-size: 11px; color: #ffffff; background: transparent;")
    _indicator = QLabel("●")
    _indicator.setStyleSheet("font-size: 14px; color: gray; background: transparent;")
    status_label = QLabel("Idle")
    status_label.setStyleSheet(f"color: {_TEXT}; background: transparent;")

    last_kw = QLabel("Last:")
    last_kw.setStyleSheet("font-weight: bold; font-size: 11px; color: #ffffff; background: transparent;")
    ...
    def _apply_status_label(text: str):
        ...
        s = text.lower()
        if "recording" in s:
            style = "font-size: 14px; color: #e74c3c; background: transparent;"
        elif "listening" in s or "wake" in s:
            style = "font-size: 14px; color: #2ecc71; background: transparent;"
        elif "processing" in s or "installing" in s or "downloading" in s:
            style = "font-size: 14px; color: #3498db; background: transparent;"
        else:
            style = "font-size: 14px; color: gray; background: transparent;"
    ...
    load_title = QLabel("SH|RA+" if is_premium() else "SH|RA")
