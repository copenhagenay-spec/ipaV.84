...
from steam import find_steam_apps, find_startmenu_apps
...
def main() -> None:
    # Single instance enforcement — prevent multiple SH|RA windows
    _mutex = None
    try:
        import ctypes
        _mutex = ctypes.windll.kernel32.CreateMutexW(None, True, "SHRASingleInstanceMutex")
        if ctypes.windll.kernel32.GetLastError() == 183:  # ERROR_ALREADY_EXISTS
            _si_app = QApplication.instance() or QApplication(sys.argv)
            QMessageBox.warning(None, "SH|RA", "SH|RA is already running.")
            return
    except Exception:
        pass

    # Write PID file so "close shira" can target this process specifically
    _pid_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "shra.pid")
    try:
    ...
    try:
        ...
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("SHRA.Assistant")
    except Exception:
        pass
    ...
    root.setWindowTitle("SH|RA+" if _is_premium() else "SH|RA")
    root.resize(620, 560)
    ...
    spotify_media = SimpleVar(value=bool(cfg.get("spotify_media", True)))
    ...
    def _fetch_latest_version() -> str | None:
        url = "https://raw.githubusercontent.com/copenhagenay-spec/Vera-beta/main/shra/VERSION"
        ...
        def _do_check():
            ...
            def _confirm_and_install():
                if not _confirm_dialog("Update Available", f"Update to v{latest}?"):
                    return
                _bridge.post(lambda: _show_ui_info("Update", "Downloading update... SH|RA will restart when ready."))
                def _do_install():
                    try:
                        data_dir = os.path.join(os.path.dirname(__file__), "data")
                        os.makedirs(data_dir, exist_ok=True)
                        installer_name = f"SHRA_Setup_{latest}.exe"
    ...
    def _record_hotkey(target_var: SimpleVar, on_done=None) -> None:
        ...
        dialog, status = _create_record_popup(
            "Record Toggle Key",
            "Press the key combination you want SH|RA to use for toggle-to-talk.",
            size="400x170",
        )
    ...
    def _record_hold_key(target_var: SimpleVar, on_done=None) -> None:
        ...
        dialog, status = _create_record_popup(
            "Record Hold Key",
            "Press the key or side mouse button you want SH|RA to use for hold-to-talk.",
            size="410x170",
        )
    ...
    def _revert():
        ...
        spotify_media.set(bool(saved.get("spotify_media", True)))
    ...
    def _start_background():
        ...
        try:
            if mode.get() == "hold":
                ...
                _start_wake_word()
                status_var.set("Wake word active (say 'shira')")
            else:
                _runtime_mode["value"] = "idle"
    ...
    def _create_tray_icon():
        ...
        icon = pystray.Icon("shra", _make_image(), "SH|RA", menu)
        return icon
    ...
    def _create_bug_report():
        ...
        # ── Submit to Discord bot (background thread — avoids freezing main thread) ─
        _BUG_REPORT_URL    = "http://147.135.6.203:8080/report"
        _BUG_REPORT_SECRET = bug_report_secret_var.get()

        _notify_info("Bug Report", "Submitting bug report, please wait...")
    ...
    def _create_shortcuts_worker():
        ...
        if desktop_ok or start_ok:
            ...
            parts.append("\nTo pin SH|RA to Start: search for SHRA, look under the 'Apps' section (not 'Best match'), right-click it, and select 'Pin to Start'.")
    ...
    def _finish_launch():
        ...
        def _auto_sync_startmenu():
            try:
                found = find_startmenu_apps()
            except Exception:
                return
            existing_names = {a.get("name") for a in apps}
            changed = False
            for app in found:
                ...
                if name not in existing_names:
                    apps.append({"name": name, "command": app["command"]})
                    existing_names.add(name)
                    changed = True
            if changed:
                _bridge.post(lambda: (_refresh_apps(), save_config(_build_config())))
        ...
        def _after_reveal():
            ...
            threading.Thread(target=_auto_sync_startmenu, daemon=True).start()
            ...
            if _is_premium() and not cfg.get("premium_onboarding_shown"):
                def _show_premium_onboarding():
                    def _dismiss_premium_onboarding():
                        ...
                        save_config(c)
                    _show_inline_notice(
                        'Welcome to SH|RA+. Themes and wake phrases are in Settings → Personality. Macros are in Integrations. Say "show help" to see everything you can do.',
                        tone="info",
                        duration_ms=0,
                        action_text="Got it",
    ...
    # Restart SH|RA automatically on system wake from sleep.
