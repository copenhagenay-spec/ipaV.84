...
def main() -> None:
    ...
    def _run_wizard():
        ...
        tk.Label(wizard, text="Welcome to IPA!").grid(row=0, column=0, columnspan=2, **w_pad)
        tk.Label(wizard, text=f"Model status: {status_text}").grid(row=1, column=0, columnspan=2, **w_pad)

        tk.Label(wizard, text="Language").grid(row=2, column=0, **w_pad)
        tk.OptionMenu(wizard, language, *LANG_CHOICES).grid(row=2, column=1, **w_pad)

        tk.Label(wizard, text="Mode").grid(row=3, column=0, **w_pad)
        tk.Radiobutton(wizard, text="Timed mic", variable=mode, value="mic").grid(row=3, column=1, **w_pad)
        tk.Radiobutton(wizard, text="Hold-to-talk", variable=mode, value="hold").grid(row=4, column=1, **w_pad)
        tk.Radiobutton(wizard, text="Hotkey", variable=mode, value="hotkey").grid(row=5, column=1, **w_pad)

        tk.Label(wizard, text="Seconds (timed/hotkey)").grid(row=6, column=0, **w_pad)
        tk.Entry(wizard, textvariable=seconds, width=10).grid(row=6, column=1, **w_pad)

        tk.Label(wizard, text="Hotkey").grid(row=7, column=0, **w_pad)
        tk.OptionMenu(wizard, hotkey, *HOTKEY_CHOICES).grid(row=7, column=1, **w_pad)

        tk.Label(wizard, text="Hold key").grid(row=8, column=0, **w_pad)
        tk.OptionMenu(wizard, holdkey, *HOLD_CHOICES).grid(row=8, column=1, **w_pad)

        tk.Checkbutton(wizard, text="Enable Spotify media controls", variable=spotify_media).grid(
            row=9, column=0, columnspan=2, padx=10, pady=2, sticky="w"
        )
        tk.Checkbutton(wizard, text="Require word 'spotify' in command", variable=spotify_requires).grid(
        ...
        tk.Button(wizard, text="Install Dependencies", command=_install_deps_wizard).grid(
        ...
        tk.Button(wizard, text="Finish", command=_finish).grid(
